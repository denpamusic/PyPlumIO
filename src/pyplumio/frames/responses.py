...
class ProgramVersion(Frame):
    ...
    def create_message(self) -> bytearray:
        ...
        struct.pack_into('<2sB2s3sBHHH', message, 0,
            data['struct_tag'],
            data['struct_version'],
            data['device_id'],
...
class CheckDevice(Frame):
    ...
    def parse_message(self, message: bytearray) -> None:
        ...
        for part in ('ip', 'netmask', 'gateway'):
            self._data['eth'][part] = (
                util.ip_from_bytes(message[offset : offset+4]))
            offset += 4

        self._data['eth']['status'] = bool( message[offset] )
        offset += 1
        for part in ( 'ip', 'netmask', 'gateway' ):
            self._data['wlan'][part] = (
                util.ip_from_bytes(message[offset : offset+4 ] ))
            offset += 4
...
class CurrentData(Frame):
    ...
    def parse_message(self, message: bytearray) -> None:
        ...
        offset += 1
        self._data['outputs'], offset = (
            structures.Outputs().from_bytes(message, offset))
        self._data['output_flags'], offset = (
            structures.OutputFlags().from_bytes(message, offset))
        self._data['temperatures'], offset = (
            structures.Temperatures().from_bytes(message, offset))
        ...
        offset += 4
        self._data['alarms'], offset = (
            structures.Alarms().from_bytes(message, offset))
        self._data['fuelLevel'] = message[offset]
        self._data['transmission'] = message[offset+1]
        self._data['fanPower'] = (
            util.unpack_float(message[offset+2 : offset+6])[0])
        self._data['boilerPower'] = message[offset + 6]
        self._data['boilerPowerKW'] = (
            util.unpack_float(message[offset+7 : offset+11])[0])
        self._data['fuelStream'] = (
            util.unpack_float(message[offset+11 : offset+15])[0])
        self._data['thermostat'] = message[offset+15]
        offset += 16
        self._data['versions'], offset = (
            structures.Versions().from_bytes(message, offset))
        self._data['lambda'], offset = (
            structures.Lambda().from_bytes(message, offset))
        self._data['thermostats'], offset = (
            structures.Thermostats().from_bytes(message, offset))
        self._data['mixers'], offset = (
...
class UID(Frame):
    ...
    def parse_message(self, message: bytearray) -> None:
        ...
        offset += 2
        self._data['UID'], offset = (
            structures.UID().from_bytes( message, offset ))
        self._data['reg_logo'] = (
            util.unpack_ushort(message[offset : offset+2]))
        offset += 2
            ...
            for parameter in range(first, parameters_number + first):
                if parameter < len(EDITABLE_PARAMS):
                    ...
                    max_ = util.unpack_ushort(
                        message[offset + 2*parameter_size : offset + 3*parameter_size])
                    if util.check_parameter(message[offset : offset + parameter_size*3]):
